...
def load_dataset_from_folders(paths: Dict[str, str], test_split_: float = 0.15, batch_size: int = 32):
    logger.warning('\n#################################')
    ...
    for dataset_name, folder_path in paths.items():
        ...
        dataset = CustomImageDataset(folder_path, transform)
        image_files.extend(dataset)
    
    total_size = len(image_files)
    val_size = int(test_split_ * total_size)
    train_size = total_size - val_size
    
    train_dataset, val_dataset = random_split(image_files, [train_size, val_size])
    
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    
    logger.warning(f'DATASET DATALOADERS CREATED')
...
train_dataload, test_dataloader = load_dataset_from_folders(config_datasetPaths, 0.15, 32)
