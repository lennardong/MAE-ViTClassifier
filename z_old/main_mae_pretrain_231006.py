...
def main(data_config: DataTrainingArguments, model_config: ModelArguments, training_config: TrainingArguments):

    ### DEPRECATION WARNINGS ###
    # TODO remove if fine
    # if model_config.use_auth_token is not None:
    #     warnings.warn("The `use_auth_token` argument is deprecated and will be removed in v4.34.", FutureWarning)
    #     if model_config.token is not None:
    #         raise ValueError("`token` and `use_auth_token` are both specified. Please set only the argument `token`.")
    #     model_config.token = model_config.use_auth_token


    ### LOGGING SETUP ###
    # TODO add logging tier here
    ...
    # Load Dataset
    ds = load_dataset_from_folders(data_config.dataset_paths, data_config.train_val_split)
    
    # NOTE testing if dataset is issue
    # ds = load_dataset("imagefolder", data_dir='./data/CAM16_100cls_10mask/test/data/normal')
    # split3 = ds["train"].train_test_split(0.25)
    # ds["train"] = split3["train"]
    # ds["validation"] = split3["test"]

    # Load Model
    config, image_processor, model = load_model(model_config)
    
    size = (image_processor.size["height"], image_processor.size["width"])
    # NOTE hardcoding
    image_column_name = get_col_names(ds, data_config, training_config)
    # image_column_name = 'image'
    transforms = Compose(
        [
    ...
    if training_config.do_eval:
        ...
        if data_config.max_eval_samples is not None:
            ds["validation"] = (
                ds["validation"].shuffle(seed=training_config.seed).select(range(data_config.max_eval_samples))
            )
    ...
    total_train_batch_size = (
        training_config.train_batch_size * training_config.gradient_accumulation_steps * training_config.world_size
    )
    if training_config.base_learning_rate is not None:
        training_config.learning_rate = training_config.base_learning_rate * total_train_batch_size / 256

    # Initialize our trainer
    trainer = Trainer(
        model=model,
        args=training_config,
        train_dataset=ds["train"] if training_config.do_train else None,
        eval_dataset=ds["validation"] if training_config.do_eval else None,
...
if __name__ == "__main__":
    ...
    # RUN
    main(data_training_arguments, model_arguments, training_arguments)
    if training_config.do_train:
        trainer = initialize_trainer()
        execute_training(trainer)
    if training_config.do_eval:
        execute_evaluation(trainer)
